...
export const listCities = async () => {
  try {
    ...
    */
    const response = await fetch(`
      ${process.env.NEXT_PUBLIC_API_BASE_URL}/pub/${getHostname()}/cities?states%5B%5D=43`,
      { method: 'GET' }
    );
...
export const listNeighborhood = async () => {
  try {
    ...
    */
    const response = await fetch(`
      ${process.env.NEXT_PUBLIC_API_BASE_URL}/pub/${getHostname()}/neighborhoods?cities%5B%5D=5300113&cities%5B%5D=5300112`,
      { method: 'GET' }
    );
